...
void generateHtml(sampleList* sList) //generate html site
{
    ...
    if ((!readTopHome) || (!readBottomHome) || (!readMiddleHome) || (!writeHome)) {
        ...
        fputs("</h2><table><thead><tr><th>Id</th><th>Link</th><th>Proccess Cnt</th><th>Dll Cnt</th><th>Memory Avg</th></tr></thead><tbody>", writeHome);
        ...
        while (sList != NULL)
        {
            ...
            sprintf(link, "%s%s%s%s %d%s", "<a class=\"link\" href=\"", createLink("sample", sList->sampleCnt), "\" target=\"_blank\">", "sample", sList->sampleCnt, "</a>");
        ...
        //write static html code
        fputs("</tbody></table></div><div class = \"right-content\"><h2>Dll's List", writeHome);
        ...
        *
        ...
        if (sList != NULL)
        {
            ...
            while (currentDll != NULL)
            {
                ...
                sprintf(link, "%s%s%s%s number %d%s", "<a class=\"link\" href=\"", createLink("dll", dllNumber), "\" target=\"_blank\">", "dll", dllNumber, "</a>");
